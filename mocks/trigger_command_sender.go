// Code generated by counterfeiter. DO NOT EDIT.
// Copyright (c) 2026 Benjamin Borbe All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.
package mocks

import (
	"context"
	"sync"

	"github.com/bborbe/github-dark-factory-watcher/pkg/command"
)

type TriggerCommandSender struct {
	SendCommandStub        func(context.Context, command.TriggerCommand) error
	sendCommandMutex       sync.RWMutex
	sendCommandArgsForCall []struct {
		arg1 context.Context
		arg2 command.TriggerCommand
	}
	sendCommandReturns struct {
		result1 error
	}
	sendCommandReturnsOnCall map[int]struct {
		result1 error
	}
	invocations      map[string][][]interface{}
	invocationsMutex sync.RWMutex
}

func (fake *TriggerCommandSender) SendCommand(arg1 context.Context, arg2 command.TriggerCommand) error {
	fake.sendCommandMutex.Lock()
	ret, specificReturn := fake.sendCommandReturnsOnCall[len(fake.sendCommandArgsForCall)]
	fake.sendCommandArgsForCall = append(fake.sendCommandArgsForCall, struct {
		arg1 context.Context
		arg2 command.TriggerCommand
	}{arg1, arg2})
	stub := fake.SendCommandStub
	fakeReturns := fake.sendCommandReturns
	fake.recordInvocation("SendCommand", []interface{}{arg1, arg2})
	fake.sendCommandMutex.Unlock()
	if stub != nil {
		return stub(arg1, arg2)
	}
	if specificReturn {
		return ret.result1
	}
	return fakeReturns.result1
}

func (fake *TriggerCommandSender) SendCommandCallCount() int {
	fake.sendCommandMutex.RLock()
	defer fake.sendCommandMutex.RUnlock()
	return len(fake.sendCommandArgsForCall)
}

func (fake *TriggerCommandSender) SendCommandCalls(stub func(context.Context, command.TriggerCommand) error) {
	fake.sendCommandMutex.Lock()
	defer fake.sendCommandMutex.Unlock()
	fake.SendCommandStub = stub
}

func (fake *TriggerCommandSender) SendCommandArgsForCall(i int) (context.Context, command.TriggerCommand) {
	fake.sendCommandMutex.RLock()
	defer fake.sendCommandMutex.RUnlock()
	argsForCall := fake.sendCommandArgsForCall[i]
	return argsForCall.arg1, argsForCall.arg2
}

func (fake *TriggerCommandSender) SendCommandReturns(result1 error) {
	fake.sendCommandMutex.Lock()
	defer fake.sendCommandMutex.Unlock()
	fake.SendCommandStub = nil
	fake.sendCommandReturns = struct {
		result1 error
	}{result1}
}

func (fake *TriggerCommandSender) SendCommandReturnsOnCall(i int, result1 error) {
	fake.sendCommandMutex.Lock()
	defer fake.sendCommandMutex.Unlock()
	fake.SendCommandStub = nil
	if fake.sendCommandReturnsOnCall == nil {
		fake.sendCommandReturnsOnCall = make(map[int]struct {
			result1 error
		})
	}
	fake.sendCommandReturnsOnCall[i] = struct {
		result1 error
	}{result1}
}

func (fake *TriggerCommandSender) Invocations() map[string][][]interface{} {
	fake.invocationsMutex.RLock()
	defer fake.invocationsMutex.RUnlock()
	copiedInvocations := map[string][][]interface{}{}
	for key, value := range fake.invocations {
		copiedInvocations[key] = value
	}
	return copiedInvocations
}

func (fake *TriggerCommandSender) recordInvocation(key string, args []interface{}) {
	fake.invocationsMutex.Lock()
	defer fake.invocationsMutex.Unlock()
	if fake.invocations == nil {
		fake.invocations = map[string][][]interface{}{}
	}
	if fake.invocations[key] == nil {
		fake.invocations[key] = [][]interface{}{}
	}
	fake.invocations[key] = append(fake.invocations[key], args)
}

var _ command.TriggerCommandSender = new(TriggerCommandSender)
