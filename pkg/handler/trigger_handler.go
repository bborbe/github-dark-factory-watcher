...
// SinglePRTriggerHandler handles POST /trigger?url=<pr_url>. The handler is
// intentionally thin: parse the URL, validate it synchronously, publish a
// TriggerCommand to Kafka, and return HTTP 202. All GitHub API access and
// candidate evaluation is owned by the in-pod command consumer.
type SinglePRTriggerHandler = libhttp.WithError

// NewSinglePRTriggerHandler returns a handler that publishes a TriggerCommand to
// Kafka for each valid /trigger request.
...
func (h *singlePRTriggerHandler) ServeHTTP(
	...
	if err := h.sender.SendCommand(ctx, command.TriggerCommand{
		URL:   rawURL,
		Force: force,
	}); err != nil {
		return libhttp.WrapWithStatusCode(
			errors.Wrap(ctx, err, "send TriggerCommand"),
...
// validateTriggerURL rejects empty URLs, unparseable URLs, and non-GitHub
// platforms with HTTP 400.
